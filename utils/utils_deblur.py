...
class MaskingOperator():
    def __init__(self, kernel_size, p):
        ...
        self.kernel_size = kernel_size
        self.mask = 1*(np.random.rand(kernel_size, kernel_size) < p).reshape(kernel_size, kernel_size)
        self.mask = np.tile(self.mask, (3,1,1))
...
class DenoisingOperator():
    def __init__(self, kernel_size):
        
        self.kernel_size = kernel_size
        self.operator = np.ones([kernel_size,kernel_size])
        self.operator = np.tile(self.operator, (3,1,1))
