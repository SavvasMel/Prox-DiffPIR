...
def main():
    ...
    noise_level_img         = 0.1       # set AWGN noise level for LR image, default: 0
    ...
    task_current            = 'deno'              # 'deno' for denoising
    n_channels              = 3                 # fixed
    cwd                     = '' 
    ...
    rate = 0.01
    result_name             = f'{testset_name}_{task_current}_{generate_mode}_{str(test_sf)}_{model_name}_sigma{noise_level_img}_NFE{iter_num}_eta{eta}_zeta{zeta}_lambda{lambda_}_rate{rate}'
        ...
        for k_index in range(k_num):
            ...
            def test_rho(lambda_=lambda_, zeta=zeta, model_output_type=model_output_type): 
                ...
                for idx, img in enumerate(L_paths):
                    ...
                    img_L = torch.tensor(img_H).to(device) # identity operator
                    

                    np.random.seed(seed=0)  # for reproducibility

                    # add poisson noise (for DPS)
                    img_L = img_L / 255
                    img_L = img_L.clamp(0, 1)
                    img_L = img_L.detach().cpu()
                    #img_L = torch.from_numpy(np.random.poisson(img_L * 255.0 * rate) / 255.0 / rate)

                    print(torch.mean(img_L * 255 * rate))

                    img_L = torch.from_numpy(np.random.poisson(img_L * 255.0 * rate))

                    img_L = img_L.to(device)
                    y = img_L.permute(2,0,1).float().unsqueeze(0)
                    ...
                    x = img_L.permute(2,0,1).float().unsqueeze(0)
                    ...
                    for i in range(len(seq)):
                        ...
                        for u in range(iter_num_U):
                                    ...
                                    if sub_1_analytic:
                                        if model_out_type == 'pred_xstart':
                                            ...
                                            if i < num_train_timesteps-noise_model_t: 
                                                x0_y = x0 / 2 + 0.5
                                                x0_y = x0_y.float()
                                                beta = np.mean(img_H)/255 * 0.01
                                                alpha = 255 * rate
                                                # x0_pidal = 1/2 * (x0_y - 1/(2*rhos[t_i].float())- beta + torch.sqrt((x0_y-1/(2*rhos[t_i].float())- beta)**2+4*y/(2*rhos[t_i].float())-2*beta/rhos[t_i].float()+4*x0_y*beta))
                                                x0_pidal =  1/2 * (x0_y - alpha/(2*rhos[t_i].float()) + torch.sqrt((x0_y-alpha/(2*rhos[t_i].float()))**2+4*y/(2*rhos[t_i].float())))
                                                x0 = x0_pidal*2-1
                                                
                                                # x0 = x0.float()
                                                # x0 = x0 / 2 + 0.5
                                                # x0 = (rhos[t_i].float()*x0 + y)/(rhos[t_i].float()+1)
                                                # x0 = 2*x0 -1 
                            ...
                            if (generate_mode == 'DiffPIR' and model_out_type == 'pred_xstart') and not (seq[i] == seq[-1] and u == iter_num_U-1):
                                ...
                                # eta_sigma = 0.8*sqrt_1m_alphas_cumprod[t_im1]/sqrt_1m_alphas_cumprod[t_i]*torch.sqrt(1-(sqrt_alphas_cumprod[t_i]/sqrt_alphas_cumprod[t_im1])**2)
                                #x = sqrt_alphas_cumprod[t_im1] * x0 + np.sqrt(1-zeta) * (torch.sqrt(sqrt_1m_alphas_cumprod[t_im1]**2 - eta_sigma**2) * eps \
                                #             + eta_sigma * torch.randn_like(x)) + np.sqrt(zeta) * torch.sqrt(sqrt_1m_alphas_cumprod[t_im1]**2 - eta_sigma**2) * torch.randn_like(x)
            ...
            # experiments
            lambdas = [lambda_*i for i in range(300,301)]
            for lambda_ in lambdas:
                #for zeta_i in [zeta*i for i in range(2,4)]:
                for zeta_i in [zeta*i*0.8 for i in range(1,2)]:
