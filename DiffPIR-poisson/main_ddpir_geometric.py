...
def main():
    ...
    sf                      = 1
    task_current            = "deblur" #"denoising" #'deblur'          
    n_channels              = 3                 # fixed
    cwd                     = ''  
    model_zoo               = os.path.join(cwd, 'model_zoo')    # fixed
    testsets                = os.path.join(cwd, 'testsets')     # fixed
    results                 = os.path.join(cwd, 'results')      # fixed
    result_name             = f'{testset_name}_{task_current}_{generate_mode}_{model_name}_rate{rate}_NFE{iter_num}_eta{eta}_zeta{zeta}_lambda{lambda_}_blurmode{blur_mode}_kernelsize{kernel_size}_geometric_tol1e-5_MIV_{0.01}'
    ...
    def test_rho(lambda_=lambda_, zeta=zeta, model_output_type=model_output_type):
        ...
        for idx, img in enumerate(L_paths):
            if task_current == 'deblur':
                ...
                raise TypeError("Choose a correct inverse problem.")
                    ...
                    if seq[i] != seq[-1]:
                        if generate_mode == 'DiffPIR':
                            if sub_1_analytic:
                                ...
                                # mu = torch.tensor(2)
                                # x0_p = utils_model.PIDAL(x0_p, FB, FBC, F2B, y, mu, rhos[t_i], alpha_bin)
                                x0_p = utils_model.LBFGSB(x0_p.ravel(), FB, FBC, shape_x0, "geometric", y.ravel(), alpha_geo, b_geo, rhos[t_i])
            ...
            if save_LEH:
                img_L = np.log(img_L+1)/np.max(np.log(img_L+1))
    ...
    for lambda_ in lambdas:
        for zeta_i in [zeta*i for i in range(5,6)]:
