...
from utils.utils_deblur import MotionBlurOperator, GaussialBlurOperator, DenoisingOperator
...
def main():
    ...
    result_name             = f'{testset_name}_{task_current}_{generate_mode}_{model_name}_rate{rate}_NFE{iter_num}_eta{eta}_zeta{zeta}_lambda{lambda_}_blurmode{blur_mode}_kernelsize{kernel_size}'
    ...
    if task_current=="deblurring":
        logger.info('Inverse problem: {}, {}, kernelsize {}'.format(task_current, blur_mode, kernel_size))
    ...
    def test_rho(lambda_=lambda_, zeta=zeta, model_output_type=model_output_type):
        ...
        if use_DIY_kernel:
            if blur_mode == 'Gaussian':
                # for reproducibility of blur kernel for each image
        ...
        if task_current == 'denoising':
            ...
            k[1,1] = 1
            # kernel = DenoisingOperator(kernel_size=kernel_size)
            # k_tensor = kernel.get_kernel()
            # k = np.copy(k_tensor)
        ...
        for idx, img in enumerate(L_paths):
            ...
            img_L = ndimage.convolve(np.float32(img_H)/255., np.expand_dims(k, axis=2), mode='wrap')
            ...
            b_poisson =  torch.tensor(np.mean(img_L * alpha_pois)*0.01).to(device)
            img_L = np.random.poisson(img_L * 255.0 * rate)
                        ...
                        if seq[i] != seq[-1]:
                            if generate_mode == 'DiffPIR':
                                if sub_1_analytic:
                                    ...
                                    #     x_deblur = np.real(np.fft.ifft2(np.asarray(FB.cpu()) * np.fft.fft2(x.reshape(1, c, m, n)))).ravel()
                                    #     return x_deblur
                                    # def TTx(x):
                                    #     x_deblur = np.real(np.fft.ifft2(np.asarray(FBC.cpu()) * np.fft.fft2(x.reshape(1, c, m, n)))).ravel()
                                    #     return x_deblur
                                    x0_p = x0 / 2 + 0.5
                                    # shape_x0 = list(x0_p.size())
                                    # print(x0_p)
                                    # raise TypeError("Only integers are allowed")
                                    # x0_p = utils_model.LBFGSB(x_hat = x0_p.ravel(), shape_in = shape_x0, k = k, measurement = y.ravel(), rho = rhos[t_i], alpha_pois = alpha_pois)
                                    ...
                                    ###############################################################
                                    mu = torch.tensor(20)
                                    x0_p = utils_model.PIDAL(x0_p, FB, FBC, F2B, y, mu, rhos[t_i], alpha_pois)
                                    x0_p = x0_p * 2 - 1
                                    ###############################################################
                                    # shape_x0 = list(x0_p.size())
                                    # obj = Objectives("Poisson", task_current, x0_p.ravel(), shape_x0, y.ravel(), alpha_pois, b_poisson, rhos[t_i], FB = FB)
                                    # der = Derivatives("Poisson", task_current, x0_p.ravel(), shape_x0, y.ravel(), alpha_pois, b_poisson, rhos[t_i], FB = FB, FBC = FBC)
            ...
            # --------------------------------
                ...
                if save_progressive:
                    ...
                    util.imsave(img_total*255., os.path.join(E_path, img_name+'_rate_{:.3f}_process_lambda_{:.3f}_{}_psnr_{:.4f}{}'.format(rate,lambda_,current_time,psnr,ext)))
    ...
    # experiments
    lambdas = [4] #[lambda_*i for i in range(7,8)]    # 3 for imagenet, 3 for ffhq
    zetas = [0.7] # [zeta*i for i in range(2,3)]
    for lambda_ in lambdas:
        for zeta_i in zetas:  # 7 for imagenet, 5 for ffhq
